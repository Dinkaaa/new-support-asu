...
function showRemovePopup(itemId, itemSrc){
	html = `<div class="modal" style="display:block">
		<div class="small-pop-up">
			...
			<button class="btn--red remove-item-btn" data-remove-id=` + itemId + ` data-remove-src = ` + itemSrc + `>Видалити</button>
			<button class="btn--green close-small-pop-up-btn">Назад</button>
		</div>
	</div>`
	$('body').append(html);
}
